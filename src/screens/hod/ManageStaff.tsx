...
export const ManageStaff = ({ navigation }: any) => {
  ...
  const deleteUser = async (id: string) => {
    Alert.alert('Delete User', 'Are you sure? This will remove the teacher from the system.', [
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View className="bg-white/5 rounded-3xl p-4 mb-3 border border-white/10 flex-row items-center">
      <View className="w-12 h-12 rounded-2xl bg-indigo-500/20 items-center justify-center mr-4">
        <Text className="text-indigo-500 font-bold">{item.email.slice(0, 2).toUpperCase()}</Text>
      </View>
      <View className="flex-1">
        <Text className="text-white font-bold" numberOfLines={1}>{item.email}</Text>
  ...
  return (
    <LinearGradient colors={['#0F0C29', '#1a1a3e', '#0F0C29']} className="flex-1">
      <View className="px-5 pt-14 pb-5 flex-row justify-between items-center border-b border-white/10">
        ...
        <Text className="text-white text-xl font-bold">Manage Teachers</Text>
        <TouchableOpacity onPress={fetchTeachers} className="w-10 h-10 rounded-full bg-white/5 items-center justify-center">
          <Ionicons name="refresh-outline" size={20} color="#6C63FF" />
        </TouchableOpacity>
      </View>

      {loading ? (
        <ActivityIndicator size="large" color="#6C63FF" className="mt-20" />
      ) : (
        <FlatList
          data={teachers}
          keyExtractor={(i) => i.id}
          ...
          ListEmptyComponent={
            <Text className="text-gray-500 text-center mt-20">No teachers found.</Text>
          }
        />
      )}
    </LinearGradient>
